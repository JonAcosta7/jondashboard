...
class CreditSpreadsApp {
    ...
    // Set up periodic API usage monitoring
    setupApiUsageMonitoring() {
        // Update usage display every 10 seconds
        setInterval(() => {
            if (this.apiManager && this.apiManager.isConfigured()) {
    ...
    loadSavedData() {
        ...
        // Set up periodic API usage updates
    ...
    setupFormListeners() {
        // Trade form buttons are now using proper onclick="app.function()" format
        console.log('Form listeners setup completed');
    }

    setupButtonListeners() {
        // Most buttons now use proper onclick="app.function()" format
        // Only setup listeners for specific cases that need it
        
        // API configuration buttons (using proper onclick already)
        // Google Drive sync buttons (using proper onclick already)
        // Data management buttons (using proper onclick already)
    ...
    async updateVixData() {
        try {
            ...
            if (vixResult.error && vixResult.needsApiKey) {
                // Handle API key not configured case
                this.vixData = {
                    level: 'API Key Required',
                    environment: 'Not Configured',
                    recommendation: 'Configure Finnhub API',
                    lastUpdate: 'Not configured',
                    analysis: 'Configure your Finnhub API key to get real VIX data for trading analysis.',
                    color: '#ed8936'
                };
            } else if (vixResult.error) {
                ...
                this.vixData = {
                    ...
                    lastUpdate: 'Failed',
                    analysis: 'Unable to fetch real VIX data. Please check your internet connection and API key.',
                    color: '#e53e3e'
                };
            } else {
                // Use real VIX data from Finnhub
                this.vixData = {
                    ...vixResult,
    ...
    async updateTrendAnalysis() {
        try {
            ...
            if (results[0].status === 'fulfilled' && !results[0].value.error) {
                ...
                hasRealData = true;
            } else if (results[0].status === 'fulfilled' && results[0].value.needsApiKey) {
                this.trendAnalysis.spyTrend = 'API Key Required';
                needsApiKey = true;
            } else {
                this.trendAnalysis.spyTrend = 'Data Unavailable';
            }
            ...
            if (results[1].status === 'fulfilled' && !results[1].value.error) {
                ...
                hasRealData = true;
            } else if (results[1].status === 'fulfilled' && results[1].value.needsApiKey) {
                this.trendAnalysis.qqqTrend = 'API Key Required';
                needsApiKey = true;
            } else {
                this.trendAnalysis.qqqTrend = 'Data Unavailable';
            }
            
            if (needsApiKey) {
                // API key not configured
                this.trendAnalysis.strength = 'Not Configured';
                this.trendAnalysis.environment = 'Configure API Key';
                this.trendAnalysis.analysis = 'Configure your Finnhub API key to get real market trend analysis.';
                this.trendAnalysis.color = '#ed8936';
            } else if (!hasRealData) {
                // No real data available
                this.trendAnalysis.strength = 'Data Unavailable';
                this.trendAnalysis.environment = 'Unable to Load';
                this.trendAnalysis.analysis = 'Unable to fetch real market data. Please check your internet connection and API key.';
                this.trendAnalysis.color = '#e53e3e';
            } else {
    ...
    async updateEconomicCalendar() {
        try {
            // Use Finnhub API for real economic calendar data
            const calendarData = await this.apiManager.fetchEconomicCalendar();
            
            if (calendarData.error && calendarData.needsApiKey) {
                this.economicCalendar = {
                    events: [],
                    riskLevel: 'Unknown',
                    advice: 'Finnhub API Key Required',
                    analysis: 'Configure your Finnhub API key to get real economic calendar data.',
                    color: '#ed8936',
                    lastUpdate: 'Not configured'
                };
                ...
                this.economicCalendar = {
                    ...
                    analysis: 'Unable to fetch economic calendar data. Please check your connection and API key.',
                    color: '#e53e3e',
                    lastUpdate: 'Failed'
                };
            } else {
    ...
    async configureFinnhubApi() {
        try {
            const apiKey = document.getElementById('finnhubApiKey').value.trim();
            
            if (!apiKey) {
                this.showError('Please enter a valid Finnhub API key');
                return;
            }
            
            // Test API key before saving
            this.showNotification('Testing API key...', 'info');
            const testResult = await this.apiManager.testApiKey(apiKey);
            
            if (!testResult.valid) {
                this.showError(`API key test failed: ${testResult.error}`);
                return;
            }
            ...
            this.apiManager.setApiKey(apiKey);
            
            // Clear the input field for security
            document.getElementById('finnhubApiKey').value = '';
            
            // Update status display
            this.updateApiKeyStatus(true);
            
            // Refresh all data with new API key
            this.performInitialDataLoad();
            
            this.showSuccess('✅ Finnhub API configured successfully!\n\nYour market data will now be fetched from Finnhub with no CORS issues.');
            
        } catch (error) {
            this.showError('Error configuring Finnhub API: ' + error.message);
        }
    }
    ...
    loadSavedApiKey() {
        try {
            ...
            let savedApiKey = localStorage.getItem('finnhubApiKey');
            
            // If no saved key, use the default one for demo/testing
            if (!savedApiKey) {
                savedApiKey = 'd1ue9dhr01qiiuq7njlgd1ue9dhr01qiiuq7njm0';
                localStorage.setItem('finnhubApiKey', savedApiKey);
                console.log('Initialized with demo Finnhub API key');
            }
            
            if (savedApiKey) {
                this.apiManager.setApiKey(savedApiKey);
                console.log('Loaded Finnhub API key');
                this.updateApiKeyStatus(true);
            }
        } catch (error) {
    ...
    updateApiKeyStatus(isConfigured) {
        ...
        if (statusElement) {
            if (isConfigured) {
                statusElement.textContent = 'Configured and ready for real market data';
                statusElement.style.color = '#38a169';
            } else {
                statusElement.textContent = 'Not configured - enter API key above';
                statusElement.style.color = '#e53e3e';
            }
        }
        
        // Update request usage display if configured
        if (isConfigured && this.apiManager) {
            this.updateApiUsageDisplay();
        }
    }
    ...
    updateApiUsageDisplay() {
        ...
        if (usageElement && this.apiManager.requestTimestamps) {
            const recentRequests = this.apiManager.requestTimestamps.length;
            const limit = this.apiManager.maxRequestsPerWindow;
            usageElement.textContent = `API Usage: ${recentRequests}/${limit} requests in last minute`;
            
            // Color code based on usage
            if (recentRequests < limit * 0.7) {
                usageElement.style.color = '#38a169'; // Green
            } else if (recentRequests < limit * 0.9) {
                usageElement.style.color = '#ed8936'; // Orange
            } else {
                usageElement.style.color = '#e53e3e'; // Red
            }
        }
    }
